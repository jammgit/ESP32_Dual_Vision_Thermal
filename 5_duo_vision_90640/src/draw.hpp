...
void draw(){
   ...
   // 绘制热点追踪十字
   if (flag_trace_max==true) {draw_cross(y_max * scale, (rows - 1 - x_max) * scale, 8);}
   // 绘制温度采集光标
...
void freeze_handeler(){
   ...
   if (flag_clear_cursor) {
      if (current_display_mode == MODE_THERMAL_ONLY) {
         draw();
      } else {
         draw_fusion();
      }
   ...
   if (flag_show_cursor && current_display_mode == MODE_THERMAL_ONLY) {
...
void screen_loop(){
   ...
   if(!flag_in_photo_mode){
      ...
      // 仅摄像头模式：跳过所有 MLX 数据处理
      if (current_display_mode == MODE_CAMERA_ONLY) {
         draw_fusion();  // 直接渲染融合模式（摄像头）
         dt = millis() - dt;
         return;
      }
      ...
      for (int i = 0; i < pixel_count; i++) {  // 拷贝温度信息, 并提前映射到色彩空间中
         // 先用一个临时 int 变量计算
      ...
      dt = millis() - dt;
      // 只在纯热成像模式显示底部状态栏，其他模式由各自的渲染函数处理
      if (flag_show_cursor==true && current_display_mode == MODE_THERMAL_ONLY){
         tft.setTextSize(1);
