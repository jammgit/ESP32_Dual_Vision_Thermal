...
void probe_error_detect(){
   ...
   if (error_count > 5){ 
      ...
      EEPROM.write(EEPROM_ADDR_SENSOR_TYPE, current_sensor); // 使用常量地址
	  EEPROM.commit();
      delay(100);
      ESP.restart();
   }
...
void probe_loop_mlx(){
    ...
    if (current_sensor == SENSOR_MLX90640){
        ...
        MLX90640_CalculateTo(mlx90640Frame, &mlx90640, emissivity, tr, currentWriteTarget);
        ...
        tr = Ta - TA_SHIFT;
        emissivity = 0.95;
        MLX90641_CalculateTo(mlx90640Frame, &mlx90641, emissivity, tr, currentWriteTarget);
    }
