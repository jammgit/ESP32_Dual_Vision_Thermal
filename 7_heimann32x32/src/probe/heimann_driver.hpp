/*** PROGRAMM INFO***************************************************************************************
  source code for HTPAd ESP32 driver
  name:           heimann_driver.hpp
  version/date:   2.2-esp32branch / 01 Jan 2025
  programmer:     written by Umeko (umeko@stu.xmu.edu.cn)
*********************************************************************************************************/
#ifndef HAIMAN_DRIVER_HPP
#define HAIMAN_DRIVER_HPP
/*** PROGRAMM INFO***************************************************************************************
...
// Note: These variables are defined in shared_val.hpp
// extern unsigned char nrofdefpix, gradscale, vddscgrad, vddscoff, epsilon, lastepsilon, arraytype;
...
void read_sensor_register(uint16_t addr, uint8_t *dest, uint16_t n)
{
  ...
  if (Wire.endTransmission(false) != 0) //Do not release bus
  {
    ...
    nBytes = Wire.requestFrom((uint8_t)SENSOR_ADDRESS, (size_t)n, (bool)true);
    if (nBytes != n)
    {
...
while (!verification_passed) {
    retry_count++;
    Serial.printf("Probe Attempt %d...\n", retry_count);

    // 1. 硬件引脚初始化
    Wire.begin(MLX_SDA, MLX_SCL);
    Wire.setClock(100000); // 建议显式设置一下频率
    
    // 2. 第一步：检查传感器地址 (0x1A) 是否存在
    Wire.beginTransmission(SENSOR_ADDRESS);
    error = Wire.endTransmission();

    // 3. 第二步：检查 EEPROM ID (0x50 - 0x0074)
    bool id_check_ok = false;
    uint32_t sensor_id = 0;

    if (error == 0) {
        // 只有传感器在线，才去查 ID，避免浪费时间
        Wire.beginTransmission(EEPROM_ADDRESS);
        Wire.write(0x00); // 地址高位
        Wire.write(0x74); // 地址低位 (E_ID1)
        int eeprom_err = Wire.endTransmission();

        if (eeprom_err == 0) {
            Wire.requestFrom(EEPROM_ADDRESS, 4); // 请求读取4字节ID
            if (Wire.available() == 4) {
                // 读取4个字节拼接成ID
                sensor_id |= ((uint32_t)Wire.read()) << 24;
                sensor_id |= ((uint32_t)Wire.read()) << 16;
                sensor_id |= ((uint32_t)Wire.read()) << 8;
                sensor_id |= ((uint32_t)Wire.read());
                
                // 【核心判断】ID 不能是 0 也就是空，也不能是全 1 (0xFFFFFFFF)
                if (sensor_id != 0 && sensor_id != 0xFFFFFFFF) {
                    Serial.printf(" >> ID Verified: 0x%08X\n", sensor_id);
                    id_check_ok = true;
                } else {
                    Serial.printf(" >> ID Invalid: 0x%08X\n", sensor_id);
                }
            }
        } else {
             Serial.println(" >> EEPROM not answering.");
        }
    }

    // 4. 第三步：如果ID也对，尝试写入配置
    if (error == 0 && id_check_ok) {
        res1 = write_sensor_byte(SENSOR_ADDRESS, CONFIGURATION_REGISTER, 0x01);
        if (res1 == 0) {
            // 全部通关！
            Serial.println(" >> Success: Hardware & ID & Config OK.");
            verification_passed = true; // 退出循环的关键
        } else {
             Serial.println(" >> Config Write Failed.");
        }
    } else {
        Serial.println(" >> Device not found or ID mismatch.");
    }
    // 如果还没通过，等待后重试
...
void sensor_init(){
  ...
  if (pixc2_0 == NULL)
  {
    Serial.println("heap_caps_malloc for hpda failed");
  }
  else
  {
    pixc2 = pixc2_0; // set pointer to start address of the allocated heap
...
//*******************************************************************
  blocking_sensor_init_and_check();
