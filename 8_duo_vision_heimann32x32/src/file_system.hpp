...
void save_align_params() {
    ...
    // 写入融合开关状态
    file.write((uint8_t)enable_fusion);

    file.close();
    Serial.println("Config (Align + Alpha + Flip + Fusion) saved");
}
...
void load_align_params() {
    ...
    // 尝试读取融合开关状态 (兼容旧配置文件)
    if (file.available() > 0) {
        enable_fusion = (bool)file.read();
    }

    file.close();
    Serial.printf("Config loaded: Align(%.2f,%.2f,%.2f,%.2f,%.2f) Alpha(%d) Fusion(%d)\n",
